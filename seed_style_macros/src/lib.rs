...
use syn::{parse_macro_input, Expr, ExprArray, DeriveInput};
use heck::SnakeCase;
use heck::KebabCase;
use syn::{
    Lit,
...
pub fn expand(input: TokenStream) -> TokenStream {
    ...
    for option in input.attrs.into_iter() {
        ...
        match option {
            ...
            Meta::NameValue(MetaNameValue{ref path, ref lit, ..}) if path.is_ident("short_prop") => {
    ...
    let variant_ident = format_ident!("{}",css_type_name.to_string().trim_start_matches("Css").to_string());
            ...
            with_themes( |borrowed_themes| {
                ...
                for (i, style_val) in &mut self.iter().cloned().map(|v| v.into()).enumerate() {
                    if let Some(breakpoint) = borrowed_themes.iter().find_map( |theme| theme.media_bp_scale.get(i) ){
                        ...
                        rules.push(Rule{value:CssValue::#variant_ident(style_val.clone())})
                    }
                }
                new_style
            })    
    ...
    impl UpdateStyle<#css_type_name> for #css_type_name {
        fn update_style(self, style: &Style)-> Style{
            ...
            let mut new_style =  style.clone();
            new_style.updated_at.push(format!("{}", Location::caller()));   
            new_style.add_rule(CssValue::#variant_ident(val));
            new_style
        }
    }
    ...
    if let syn::Data::Enum(data_enum) = input.data {
        let count = data_enum.variants.iter()
        .filter(|v| v.fields == syn::Fields::Unit)
        .filter(|v| {
            v.ident.clone().to_string() != "Inherit".to_string()
        })
        .filter(|v| {
            v.ident.clone().to_string() != "Initial".to_string()
        }).count();
        if count > 0 {
            create_extension_trait = true;
        }

        let func_impls_defn = data_enum.variants.iter()
           .filter(|v| v.fields == syn::Fields::Unit)
        .filter(|v| {
            v.ident.clone().to_string() != "Inherit".to_string()
        })
        ...
        .map(|v| {
            ...
            let value_variant_name = format_ident!("{}", css_type_name.clone().to_string().trim_start_matches("Css").to_string());
    ...
    let func_impls = data_enum.variants.iter()
    .filter(|v| v.fields == syn::Fields::Unit)
    .filter(|v| {
        v.ident.clone().to_string() != "Inherit".to_string()
    })
    ...
    .map(|v| {
        ...
        quote!{
            fn #short_prop_ident(&self) -> Style {
                ...
                new_style.updated_at.push(format!("{}", Location::caller()));
                new_style.add_rule(CssValue::#value_variant_name(#css_type_name :: #f_big_name));
                new_style
            }
                    fn #f_small_name_ident(&self) -> Style {
                let mut new_style = self.clone();
...
for property  in properties.elems.iter() {
 ...
 if let Expr::Tuple(ref tuple) = property {
     ...
     if let (Expr::Lit(ref theme_id), Expr::Lit(ref generic_type_name)) =  (theme_id, generic_type_name){
...
pub fn generate_from_strs(input: TokenStream) -> TokenStream {
    ...
    let GenerateFromStr {
               properties,
           } = parse_macro_input!(input as GenerateFromStr);
...
pub fn create_pseudos(input: TokenStream) -> TokenStream {
    ...
    let CreatePseudos {
               properties,
           } = parse_macro_input!(input as CreatePseudos);
...
pub fn generate_froms(input: TokenStream) -> self::proc_macro::TokenStream {
    ...
    let CreateFromTraits {
               properties,
           } = parse_macro_input!(input as CreateFromTraits);
    ...
    for property  in properties.elems.iter() {
        ...
        if let Expr::Tuple(ref tuple) = property {
            ...
            if let (Expr::Lit(ref theme_id), 
                    ...
                    if let (syn::Lit::Str(ref theme_ids), 
                    ...
                    let variant_ident = format_ident!("{}", specific_type_names.clone().value().trim_start_matches("Css").to_string());
                ...
                impl UpdateStyle<#specific_ident> for usize {
                    fn update_style(self, style: &Style)-> Style {
                         ...
                         with_themes( |borrowed_themes| {
                            ...
                            let mut new_style =  style.clone();
                            new_style.updated_at.push(format!("{}", Location::caller()));   
                            new_style.add_rule(CssValue::#variant_ident(theme_value));
                            new_style
                         })
                    }    
            }
...
pub fn create_enums(input: TokenStream) -> TokenStream {
    ...
    let CreateEnums {
               properties,
           } = parse_macro_input!(input as CreateEnums);
